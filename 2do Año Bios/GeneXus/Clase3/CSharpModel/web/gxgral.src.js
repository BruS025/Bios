...
/**@preserve GeneXus 15.0.6.116598*/
		...
		evt: {
			...
			fireControlValueChange: function(gxO, Ctrl, evt) {
				...
				if (vStruct && vStruct.evt_cvc) {
					deferred = gxO[vStruct.evt_cvc].call(gxO);
					if (deferred) { //Client side events does not return Promises yet. 
			...
			fireControlValueChanging: function(gxO, Ctrl, callback) {
				if (!gxO || !Ctrl) //This should be an error..
					return;
				var vStruct = gxO.getValidStructFld(Ctrl);
				if (vStruct && vStruct.evt_cvcing && !vStruct.gxsgprm) { //Not supported when Suggest is ON.			
					...
					var deferred = gxO[vStruct.evt_cvcing].call(gxO);
					if (deferred) { //Client side events does not return Promises yet. 						
			...
			onkeypress: function (xevent, hasenter, skiponenter) {
				...
				if (gx.grid.handleKeyPressEvt(event))
					return;
			...
			onchange_impl: function(Ctrl, event) {				
				gx.evt.setReady(false);
		...
		objectLoad: function (gxGrids, gxHiddens) {
			...
			gx.plugdesign.init().done(function() {
				if (typeof(gx.StorageTimeZone) != 'undefined' && gx.StorageTimeZone != gx.NULL_TIMEZONEOFFSET) {
					...
					if (!gx.lang.emptyObject(timezone) && gx.http.getCookie('GxTZOffset') != timezone  && gx.http.setCookie('GxTZOffset', timezone, 180, '/'))
						gx.http.reload();
				}
...
gx.plugdesign = (function($) {
	...
	return {
		...
		applyClassMaps: function (class_maps, selector, excluded, classMapSelector) {
		...
		applyTemplateObject: function(opts) {
			...
			if (!templatesLoaded) {
				...
				this.applyTemplates(templates, selector, excluded, templateSelector);
				this.applyClassMaps(class_maps, selector, excluded, classMapSelector);
				gx.fx.obs.notify('gx.plugdesign.onafterapplytemplate');
				deferred.resolve();
			}
			return deferred.promise();
		...
		Template: (function () {
			var template = function (opts) {
				...
				this.apply = function (el, context) {
					...
					{
						compiledTemplate = Mustache.compile(t.preProcessTemplate());
					}
					if (compiledTemplate) {
		...
		gxdate: function (SDate, XSFmt) {
			...
			this.compare_date = function (DateValue) {
				...
				if (DateValue instanceof gx.date.gxdate)
					...
					Val = DateValue;
				if (this.HasTimePart || DateValue.HasTimePart)
				...
				var close_impl = function () {
					...
					if ((cParms != null) && (this.IsPrompt)) {
						if (gxO && opts.parmsMetadata && opts.updateParms) {
							...
							for (i = 0; i < len; i++) {
								cParms[i] = gxO[opts.parmsMetadata[i]];
							}
						}
					...
					gx.ajax.dispatchCommands(this.OncloseCmds, undefined, { updateParms: true });
					this.cleanup();
					this.state = 'closed';
					if (this.afterClose) {
	...
	return {
		...
		dispatchRefreshCommand: function (command, gxO) {
			...
			if (refreshMethod == 'GET') {
				...
				gxOEvent.executeServerEvent(rfrName, true, null, true, isNoGridEvent);	
			}
		},

		dispatchCommands: function (Commands, gxO, opts) {
			opts = opts || {};
			var redirect,
				tempGxO,
				oldgxO,
	...
	return {
		...
		grid: function (parentObj, gLvl, gLvlName, gId, gName, rgName, cName, gCmpCtx, gIsInMaster, pName, gKey, gFreestyle, gCols, aSelect, aHover, gRows, gPaging, 
			...
			this.infinite_scrolling_after_scroll = function() {
				...
				if (this.grid.newAdditiveRows) {
					$(this.grid.newAdditiveRows).show();
				}
			}
			...
			this.unInstallScrollListener = function() {
				if (this.ScrollingElement) {
					...
					if (this.ScrollType == SCROLL_FORM) {
							...
							$(this.ScrollingElement).css({'height': 'auto'}).removeClass(GRID_INFINITE_SCROLLING_ELEMENT_CLASS);
					}
				}
			}
			...
			this.installScrollListener = function(ScrollingElement, gridTblid, $body, selector) {
				var scrollNextDemand = this.InverseLoading ? 	function() { return ScrollingElement.scrollTop <= ScrollingElement.clientHeight * 30 / 100} :
																function() { return ScrollingElement.scrollTop/(ScrollingElement.scrollHeight - ScrollingElement.clientHeight) > 0.6;};
				var _this = this;
			...
			this.setSelection = function (idx, fireEvt) {
				if (this.allowSelection && idx >= 0) {					
					...
					if (rowCtrl) {
						this.grid.setRowSelected(rowCtrl, gx.grid.rowId(idx + 1), undefined, undefined, fireEvt);
						return true;
					}
				}
				return false;
			...
			this.refreshGrid = function (ops) {
				...
				var afterRender = function () {
					...
					var _this = this, 
						applyInfiniteScrolling = function(arrCmpObj) {
								var fnc = function() {
									if (gx.spa.isNavigating()) {
					...
					this.setupGridComponents(this.GridComponents).then( function() {
							...
							if (_this.isNestedLoad) {
								...
								if (_this.additiveResponse) {
									...
									_this.applyTemplateObject().then( function() { applyInfiniteScrolling(arrCmpObj)});
								}
							}
					});
...
gx.grid.impl = (function ($) {
	return function (id) {
		...
		this.keepGridItemVisible = function (idx) {
			...
			if ($scrollingElement.length > 0 && $scrollingElement[0].offsetHeight < $scrollingElement[0].scrollHeight) {
				$scrollingElement.css('position', 'relative');
				
				var rowCtrl = this.ownerGrid.getRowCtrlByIdx(idx);
				if (rowCtrl) {
					var $ctrl = $(rowCtrl),
						scrollingElement = $scrollingElement[0],
						elementY = $ctrl[0].offsetTop,
						elementHeight = $ctrl.outerHeight(true),
						paddingTop = (isOldIE)? parseInt($container.css('paddingTop'), 10): 0;
					
					$scrollingElement.css('position', '');
					
					if (scrollingElement.scrollTop > elementY ) {
						$scrollingElement[0].scrollTop = elementY;
					}
					if (elementY + elementHeight + paddingTop > scrollingElement.scrollTop + scrollingElement.offsetHeight) {
						$scrollingElement[0].scrollTop = elementY - scrollingElement.offsetHeight + elementHeight + paddingTop;
					}	
				}
			}
		}
		...
		this.setNextRowHovered = function () {
			return this.toggleRowHoverById(this.gxHoveredRowId + 1, true);
		}
		...
		this.toggleRowHoverById = function (idx, enabled) {
			var rowCtrl = this.ownerGrid.getRowCtrlByIdx(idx),
				handled = false;
			if (rowCtrl) {
				handled = true;
...
gx.GxObject = (function($) {
	var gxObject = function () {
		...
		var dispatchControlEvent = function (domEvt) {
			if (domEvt.type === "keypress" && (domEvt.keyCode !== 13/* && domEvt.keyCode !== 8*/)) { //Only enter Event (for keypress Evt) should be processed here.
				return; 
			}
		...
		this.handleControlEvent = function (domEvt, fld, el) {
			...
			var vStruct = controlEventMap[fld], 				
				...
				jsScriptType = $(el).attr(GX_EVENT_DATA_ATTR) || (readOnlyCtrl? $(el).find("> a").first().attr(GX_EVENT_DATA_ATTR): ''),
				jsScriptCode = $(el).attr(GX_EVENT_CODE_DATA_ATTR) || (readOnlyCtrl? $(el).find("> a").first().attr(GX_EVENT_CODE_DATA_ATTR): ''),
		...
		this.onload = function (loadGrids, forceAfterLoad) {
			...
			gx.dbg.logPerf('objectOnload_' + this.CmpContext + this.ServerClass);
			var _this = this;
			$.when.apply($, arrDeferreds).done(function () { _this.onLoadDeferred.resolve()});			
		};
...
gx.fn = (function($) {
	...
	return {
			...
			try {
				if (window.parent && window.parent.gx) {
					if (gx.popup.ispopup() && !(gx.grid.drawAtServer || window.parent.gx.grid.drawAtServer))
						gx.fn.closeWindow(gx.json.evalJSON(Parms), { parmsMetadata: gx.json.evalJSON(ParmsMetadata) });
					else
						window.parent.gx.fn.closeWindow(gx.json.evalJSON(Parms), { parmsMetadata: gx.json.evalJSON(ParmsMetadata) });
					return true;
				}
			}
			catch (e) {
		...
		getRONode: function (id, create) {
			...
			span.appendChild(document.createTextNode(gx.fn.getControlValue(gx.dom.id(el), 'screen') || ''));
		...
		objectOnload: function (loadGrids, forceAfterLoad) {
			...
			$(document).on('keyup', 'input[type="text"], textarea', function (event) {
