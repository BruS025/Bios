    ...
    internal class PersistenciaLineas
    {
        internal void AgregarLinea(Linea linea, int idFactura)
        {
            ...
            try
            {
                ...
                SqlParameter _Cant = new SqlParameter("@Cantidad", linea.Cantidad);
        ...
        public List<Linea> BuscarLinea(int id)
        {
            ...
            try
            {
                ...
                while (oReader.Read())
                {
                    Resultado.Add(new Linea(new Articulo(Convert.ToInt32(oReader["CodArt"]), oReader["NomArt"].ToString(), Convert.ToDecimal(oReader["PreArt"])),
                                                   Convert.ToInt32(oReader["Cantidad"])));
                }
                oReader.Close();
            }
            catch (Exception)
