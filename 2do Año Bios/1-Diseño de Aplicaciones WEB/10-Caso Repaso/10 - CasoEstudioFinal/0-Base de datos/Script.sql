...
Begin
		...
		If @@ERROR = 0
			return ident_current ('Cliente') --@@Identity devuelve el ultimo autogenerado en la bd
		else
			return -2
End
go
...
Begin
		If Not (Exists(select * from Cliente where NumCli = @NumCli))
			return -1
		if exists (select * from TelefonosCliente where NumCli = @NumCli)
			return -3
...
Begin
		...
		--1 definir el tipo de cuenta
		--cuenta corriente saldo - monto retiro >= minimo
		--caja de ahorro saldo - monto retiro >= 0
		--no dejar que llege al update !!!!!!!!
		/*	if (select * from CuentaCAhorro where NumCta = @NumCta)
				begin
					declare @varr int
					set @varr = */
