   ...
   public class PersistenciaCuentaCAhorro
    {
       ...
       public static void Alta(CuentaCAhorro pCuenta)
       {
           ...
           _comando.Parameters.AddWithValue("@CICli", pCuenta.UnCliente.CICli);
       ...
       public static CuentaCAhorro Buscar(int pNumCta)
       {
           ...
           try
           {
               ...
               {
                   _lector.Read();
                   _unaCta = new CuentaCAhorro((int)_lector["NumCta"], (PersistenciaCliente.ClienteBuscar((int)_lector["CICli"])), Convert.ToDouble(_lector["SaldoCta"]), Convert.ToInt32(_lector["MovsCta"]), Convert.ToDouble(_lector["CostoMovCta"]));
               }
           }
           catch (Exception ex)
           {
       ...
       public static List<CuentaCAhorro> CuentaCAhorroListado()
        {
            ...
            try
            {
                ...
                if (_lector.HasRows)
                {
                    while (_lector.Read())
                    {
                        _unaCta = new CuentaCAhorro((int)_lector["NumCta"], PersistenciaCliente.ClienteBuscar((int)_lector["CICli"]), Convert.ToDouble(_lector["SaldoCta"]), (int)_lector["MovsCta"], Convert.ToDouble(_lector["CostoMovCta"]));
                        _lista.Add(_unaCta);
                    }
                }
                _lector.Close();
