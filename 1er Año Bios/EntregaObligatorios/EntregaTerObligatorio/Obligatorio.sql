...
if exists (select PC.CodigoCongelado from ProductosCongelados PC where PC.CodigoCongelado=@CodigoCongAgregar union select PE.CodigoEnlatados from ProductosEnlatados PE where PE.CodigoEnlatados=@CodigoCongAgregar)
	...
	begin transaction	
				if @PrecioAgregar <=0
					begin
						rollback tran
							select 'Cod.Del.Error==> ' + cast(@@Error as nvarchar(20)) + 'El precio tiene que ser mayor que 0'
					end
...
go
declare @retorno int
exec @retorno = AgregarProductoCongelado prueba,0,'20161028',1,1,222
print @retorno
go
...
create proc TotRecaudadoPorProd
...
exec TotRePorProd '20161025','20161128',66778
go
...
select * from ProductosCongelados
